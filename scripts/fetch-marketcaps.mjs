// Fetches market value and trailing revenue for every company in data/companies.json
// and writes data/marketcaps.json.
//
// This runs in GitHub Actions rather than in the browser: Yahoo does not send
// CORS headers, and the crumb handshake below needs a cookie jar. The page just
// reads the committed JSON, so a failed run leaves yesterday's numbers in place
// rather than emptying the charts.
//
// A company that cannot be priced is simply left out of the file. The page
// counts what is missing and says so, which is the honest failure mode --
// better a chart that admits a gap than one silently drawn from stale figures.
...
await writeFile(new URL('../data/marketcaps.json', import.meta.url), JSON.stringify(out, null, 2) + '\n');
