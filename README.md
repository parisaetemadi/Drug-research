...
Fourteen sections, one chart each:
...
| **04 · Inside a trial** | What each phase asks for: patients, sites, duration and cost |
| **05 · Where the supply is thin** | Single-country and single-supplier concentration for specific ingredients |
| **06 · Made everywhere, invented somewhere** | API manufacturing sites against the origin of new drug programmes |
| **07 · What one medicine costs** | US list prices, from $4 to $4.25 million, on a log scale |
| **08 · How long the monopoly lasts** | The patent clock against regulatory exclusivity, on one axis |
| **09 · What stops them charging anything** | Medicare's first negotiated prices, and how other countries do it |
| **10 · Who actually works here** | The ten largest employers by headcount, coloured by layer |
...
- **Curated** — `layers.json`, `companies.json`, `gauntlet.json`, `trials.json`,
  ...
  `pricing.json`, `jobs.json`. Each file carries a `sources` array, and those
  strings are what the page prints in its own footer. Editing a number means
  editing its source alongside it.
- **Generated** — `world.json`, country outlines simplified from Natural Earth
  1:110m (public domain) by `tools/build-world.py`. Rebuild it only if the list
  of countries with a head office in it changes. Note that this Natural Earth
  extract carries the United Kingdom as Great Britain alone.
- **Live, with a fallback** — `marketcaps.json`. Market value, revenue and
  margins are read from Yahoo when the page opens, by way of a function on the
  site that links here (see below). The file holds rounded order-of-magnitude
  market values so the treemap has a shape before that lands, and so it still
  has one if the read fails — in which case the section says so on the chart
  itself in a box you cannot miss. A treemap of provisional numbers looks
  exactly like a real one, so that caveat does not live in a footnote.
...
Cloudflare reaches Yahoo from addresses it will serve. So the live figures come
from a Pages function on the site that links to this page —
`/api/fundamentals`, which fetches `quoteSummary` per symbol, converts foreign
listings to dollars, and returns the result with an
`Access-Control-Allow-Origin` header for this page's origin. `app.js` calls it
in batches of 36 (Cloudflare's free plan allows 50 subrequests per invocation)
and merges whole quotes over the placeholders, never half of one.

If that endpoint is unreachable — offline, a changed origin, Yahoo refusing —
nothing breaks: the placeholders stand and the chart says they are placeholders.
...
- Section 06's two columns come from different sources counting different things.
